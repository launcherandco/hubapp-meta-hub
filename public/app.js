...
(function () {
  ...
  var YT_URL = "https://hubapp.pro.br/";
  var ZP_URL = "https://hubapp.pro.br/";
  ...
  // ── YouTube channel config (guide) ─────────────────────────
  var _ytChannelUrl = "";
  function applyYtChannel(url) {
    _ytChannelUrl = url || "";
    var inp = $("cfgYtChannel"); if (inp) inp.value = _ytChannelUrl;
    renderYtChannel();
  }
  function renderYtChannel() {
    var area = $("ytChannelArea"); if (!area) return;
    if (!_ytChannelUrl) {
      area.innerHTML = '<p class="hint" style="margin:0">' + esc(t("guide.videos.empty")) + '</p>';
      return;
    }
    var plm = _ytChannelUrl.match(/[?&]list=([A-Za-z0-9_-]+)/);
    if (plm) {
      area.innerHTML = '<div class="yt-embed-wrap"><iframe src="https://www.youtube.com/embed/videoseries?list=' + encodeURIComponent(plm[1]) + '&rel=0&modestbranding=1" title="YouTube" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
      return;
    }
    var vidm = _ytChannelUrl.match(/(?:youtu\.be\/|[?&]v=|\/embed\/)([A-Za-z0-9_-]{6,})/);
    if (vidm) {
      area.innerHTML = '<div class="yt-embed-wrap"><iframe src="https://www.youtube.com/embed/' + encodeURIComponent(vidm[1]) + '?rel=0&modestbranding=1" title="YouTube" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
      return;
    }
    area.innerHTML = '<a class="btn" href="' + escAttr(_ytChannelUrl) + '" target="_blank" rel="noopener">' + esc(t("guide.videos.visitChannel")) + '</a>';
  }
  function initYtChannelConfig() {
    var btn = $("ytCfgToggle"), form = $("ytCfgForm"), save = $("saveYtChannel");
    if (btn && form) btn.addEventListener("click", function () {
      var open = form.classList.toggle("hidden"); btn.setAttribute("aria-expanded", !open ? "true" : "false");
    });
    if (save) save.addEventListener("click", function () {
      var url = ($("cfgYtChannel") || {}).value || "";
      save.disabled = true;
      api("/api/settings", { method: "POST", body: { youtubeChannelUrl: url } })
        .then(function (c) { applyYtChannel(c.youtubeChannelUrl || ""); toast(t("config.saved")); })
  ...
  function renderSidePanel() {
    ...
    if (hasApp && hasVerify && hasChannel) {
      box.innerHTML =
        ...
        '<a href="' + ZP_URL + '" target="_blank" rel="noopener" style="display:inline-block;margin:.2rem 0 .85rem;font-weight:800;font-size:.95rem;color:#1faa53;text-decoration:none">hubapp.pro.br</a>' +
  ...
  function loadConfig() {
    api("/api/config").then(function (c) {
      ...
      applyYtChannel(c.youtubeChannelUrl || "");
      fillGuideUrls();
    }).catch(function (e) { toast(e.message, true); });
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("click", function (e) {
      ...
      if (tb) { var w = window.open(tb.getAttribute("data-embed-test"), "hubapp_connect", "width=560,height=740"); if (!w) toast(t("embed.popupBlocked"), true); return; }
